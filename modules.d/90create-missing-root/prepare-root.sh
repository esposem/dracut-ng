...
A /.overlay/work - - - - system_u:object_r:root_t:s0" > /run/tmpfiles.d/verity.conf

echo "d /var/log/audit 700 root root -
d /var/lib/rsyslog 700 root root -
A /var/log/audit - - - - system_u:object_r:auditd_log_t:s0" >> /run/tmpfiles.d/verity.conf

systemd-sysusers --root $NEWROOT
systemd-tmpfiles --root $NEWROOT --create
systemd-tmpfiles --root $NEWROOT --create /run/tmpfiles.d/verity.conf
